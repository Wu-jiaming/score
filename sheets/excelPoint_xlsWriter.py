...
def xlsxFilesWrite(xlsxDir, type, headerLists, valueLists):
	#新建一个xlsx文件
	workBook = xlsxwriter.Workbook('cm-data.xlsx')
	#group[1]代表着dict的数据
	#group[0]代表list文件名
	...
	for a in range(len(group[1])):
		...
		print("b", group[0][a])
		#print(str(1)+'.xlsx')
		#xlsxFileName = xlsxDir + str(group[0][a])
		sheetName = str(group[0][a])
		valueL = group[1][a]
		print("========")
		print("valueL:", valueL)
		print("------------")
		xlsxChart(workBook, sheetName, type , headerLists, valueL)
	workBook.close()
...
新建xlsx文件，保存数据到excel
"""
def xlsxChart(workBook, sheetName, type, headerLists, valueLists):
	workBook = workBook
	#新建一个sheet,名字为SheetName,workSheet是名字为sheetName的sheet对象
	...
	chart_col = chartType(fileName, type, workBook, endIndex)
	#插入图表
...
在excel表中插入图标的样式
"""
def chartType(sheetName, type, workBook, endIndex):
	#xlsx文件添加一个图表，类型是折线图
	chart_col = workBook.add_chart({'type':type})
	chartAddSeries(sheetName, chart_col, endIndex)
...
不同图表有不同图表series参数
"""
def chartAddSeries(sheetName, chart_col, endIndex):
		#添加第一个折线图
